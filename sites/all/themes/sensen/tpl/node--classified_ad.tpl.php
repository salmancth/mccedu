...
if ($teaser) { ?>    
        ...
        <?php 
        $term_name = ''; $term_image_url= ''; $term_url = '';
        if (!empty($node->field_classified_ad_category)) {
            $term = taxonomy_term_load($node->field_classified_ad_category[LANGUAGE_NONE][0]['tid']) ;
            $term_image_url = file_create_url($term->field_image_front[LANGUAGE_NONE][0]['uri']);  
            $term_url = drupal_get_path_alias('taxonomy/term/' . $term->tid); 
            if ($term->name) $term_name = $term->name;
        }
        ?>

        <li class="item col-md-12 hide-comment-box">
            <div class="content_out clearfix">
                <div class="bk-mask">
                    <div class="thumb hide-thumb"> <a href="<?php print $node_url;?>"> <img src="<?php print $term_image_url; ?>" alt="<?php $term_name;?>"> </a> <!-- close a tag --></div>
                    <!-- close thumb --> </div>
                <div class="post-c-wrap">
                        <div class="meta">
                            <div class="post-category"> <i class="fa fa-sitemap"></i> <a href="<?php print $term_url;?>"> <?php print $term->name;?></a> </div>
                    ...
                    <h4 class="title"><a href="<?php print $node_url; ?>"><?php print $node->title; ?></a></h4>
                    <div class="excerpt">
                        <?php
                            $summary_strip_tags = strip_tags($node->field_summary[LANGUAGE_NONE][0]['value']);                
                            print substr($summary_strip_tags,0,300) . ' ...';
                        ?> </div>
                    <div class="readmore"><a href="<?php print $node_url; ?>"><?php print t('Read More') ?></a></div>
                </div>
                <div class="post-meta ">
                    <div class="post-meta-reads">
                        <div class="reads-label">Viewed</div>
    ...
    <?php } else {
        ...
        <div class="s_header_wraper">
            <div class="s-post-header container">
                  ...
                  <?php print render($title_suffix); ?>
                <div class="meta">
                    <div class="post-category"> <i class="fa fa-sitemap"></i> <?php print strip_tags(render($content['field_classified_ad_category']), '<a>'); ?></div>
                    <div class="post-date" itemprop="datePublished"><i class="fa fa-clock-o"></i> <?php print format_date($node->changed, 'custom', 'M d Y h:i A'); ?></div>
                    <div class="post-author" itemprop="author"> <i class="fa fa-user"></i> <?php print t("by") ?><?php print $name ?></div>
                    <div class="post-reads bg-primary p5 " itemprop="postTotalReaders"><i class="fa fa-book"></i> <?php print t('Reads ') . intval($statistics['totalcount']); ?></div>
                </div>
        ...
        <?php if (empty($node->field_hide_page_meta_links[LANGUAGE_NONE][0]['value'])) : ?>
            <?php
                $content['field_classified_ad_category']['0']['#title'] = t('See more articles like this');
            ?>
                    ...
                    <div class="row comment-box-bottom">
                        <div class="col col-sm-4">
                            <div class="featured-box small-box icon-main-color box-none box-top-left">
                                ...
                                <p class="box-link"><?php print strip_tags(render($content['field_classified_ad_category']), '<a>'); ?></p>
                            </div>                
                        </div>
                        <div class="col col-sm-4">
                            <div class="featured-box small-box icon-main-color box-none box-top-left">
